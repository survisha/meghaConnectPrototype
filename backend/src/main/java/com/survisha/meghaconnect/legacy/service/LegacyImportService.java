...
public class LegacyImportService {
    ...
    @Value("${legacy.import.max-file-size-mb:50}") private long maxFileSizeMb;
    ...
    public BatchSummary uploadAndAnalyze(MultipartFile file, String actor) {
        ...
        try (InputStream in = Files.newInputStream(stored); Workbook workbook = WorkbookFactory.create(in)) {
            ...
            batch.setOverallStatus(mappingRequired > 0 ? "READY_FOR_MAPPING" : "READY");
    ...
    public BatchSummary execute(Long batchId,String actor,boolean admin){
        LegacyImportBatch batch=ownedBatchForUpdate(batchId,actor,admin);
        if(Set.of("IMPORTING","COMPLETED","PARTIAL_SUCCESS","FAILED").contains(batch.getOverallStatus()))throw new MeghaConnectException("LEGACY_IMPORT_ALREADY_EXECUTED","This workbook import is already running or has finished.",409);
        List<LegacyImportSheet> sheets=sheetRepository.findByBatchIdOrderBySheetIndex(batchId);validateReadyForImport(sheets);
        batch.setOverallStatus("IMPORTING");batch.setStartedAt(now());batchRepository.save(batch);
        audit.log("LegacyImportBatch",batchId,"LEGACY_IMPORT_STARTED","Workbook import started",actor);
        for(LegacyImportSheet sheet:sheets){
            if("SKIPPED".equals(sheet.getStatus()))continue;
    ...
    private void validateReadyForImport(List<LegacyImportSheet> sheets){List<LegacyImportSheet>active=sheets.stream().filter(s->!"SKIPPED".equals(s.getStatus())).collect(Collectors.toList());List<LegacyImportSheet>incomplete=active.stream().filter(s->!"READY".equals(s.getStatus())||s.getConfirmedDataset()==null).collect(Collectors.toList());if(active.isEmpty())throw new MeghaConnectException("LEGACY_IMPORT_NO_IMPORTABLE_SHEETS","At least one sheet must be ready to import.",409);if(!incomplete.isEmpty())throw new MeghaConnectException("LEGACY_IMPORT_MAPPING_INCOMPLETE","Mapping is incomplete for "+incomplete.size()+" sheet"+(incomplete.size()==1?".":"s."),409);for(LegacyImportSheet sheet:active)ensureMandatoryMapped(sheet.getConfirmedDataset(),columnRepository.findBySheetIdOrderBySourceColumnIndex(sheet.getId()));}
    private void recalculateBatch(Long id){LegacyImportBatch b=batchRepository.findById(id).orElseThrow();List<LegacyImportSheet>s=sheetRepository.findByBatchIdOrderBySheetIndex(id);b.setImportedSheets((int)s.stream().filter(x->Set.of("COMPLETED","PARTIAL_SUCCESS").contains(x.getStatus())).count());b.setFailedSheets((int)s.stream().filter(x->"FAILED".equals(x.getStatus())).count());b.setSkippedSheets((int)s.stream().filter(x->"SKIPPED".equals(x.getStatus())).count());b.setMappingRequiredSheets((int)s.stream().filter(x->"MAPPING_REQUIRED".equals(x.getStatus())).count());b.setTotalRows(s.stream().mapToLong(LegacyImportSheet::getTotalRows).sum());b.setValidRows(s.stream().mapToLong(LegacyImportSheet::getValidRows).sum());b.setImportedRows(s.stream().mapToLong(LegacyImportSheet::getImportedRows).sum());b.setFailedRows(s.stream().mapToLong(LegacyImportSheet::getFailedRows).sum());b.setDuplicateRows(s.stream().mapToLong(LegacyImportSheet::getDuplicateRows).sum());boolean ready=s.stream().anyMatch(x->"READY".equals(x.getStatus()));if(b.getMappingRequiredSheets()>0)b.setOverallStatus("READY_FOR_MAPPING");else if(b.getImportedSheets()>0&&(b.getFailedSheets()>0||b.getSkippedSheets()>0||b.getFailedRows()>0||b.getDuplicateRows()>0))b.setOverallStatus("PARTIAL_SUCCESS");else if(b.getImportedSheets()>0)b.setOverallStatus("COMPLETED");else if(b.getFailedSheets()>0)b.setOverallStatus("FAILED");else if(ready)b.setOverallStatus("READY");else b.setOverallStatus("READY_FOR_MAPPING");batchRepository.save(b);}
    ...
    private LegacyImportBatch ownedBatchForUpdate(Long id,String actor,boolean admin){LegacyImportBatch b=batchRepository.findByIdForUpdate(id).orElseThrow(()->new NoSuchElementException("Import batch not found."));if(!admin&&!b.getUploadedBy().equalsIgnoreCase(actor))throw new AccessDeniedException("You cannot access this import batch.");return b;}
    ...
    private void validateUpload(MultipartFile f){if(f==null||f.isEmpty())throw new IllegalArgumentException("Select a non-empty Excel workbook.");if(f.getSize()>maxFileSizeMb*1024*1024)throw new IllegalArgumentException("Legacy Excel file exceeds the maximum allowed size of "+maxFileSizeMb+" MB.");String name=Optional.ofNullable(f.getOriginalFilename()).orElse("").toLowerCase(Locale.ROOT);if(!(name.endsWith(".xlsx")||name.endsWith(".xls")))throw new IllegalArgumentException("Only .xls and .xlsx files are accepted.");if(f.getContentType()!=null&&!EXCEL_TYPES.contains(f.getContentType()))throw new IllegalArgumentException("The uploaded MIME type is not an Excel workbook.");}
