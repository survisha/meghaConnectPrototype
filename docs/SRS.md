...
| VR-007 | Visitor registration includes explicit Previous/Next step navigation and a Home link back to login |
| VR-008 | Public users see a visitor dashboard showing own records |
...
| SCH-005 | Drag-and-drop reschedule available for OSD/Admin; HCM can drag-upgrade. Uses Angular CDK Drag & Drop with visual feedback (drag handles, preview, placeholder) |
...
| SCH-008 | Angular Material calendar provides date selection with visual indicators for dates containing events |
| SCH-009 | Demo mode: When API returns no data or HCM logs in without backend connectivity, system displays dummy schedule events from MockDataService for demonstration purposes |
...
**Frontend Implementation:** The scheduling interface uses Angular Material's `mat-calendar` component combined with Angular CDK Drag & Drop module for a rich, interactive calendar experience.

**Key Features:**
- **Date Picker:** Left sidebar with `mat-calendar` for date selection
- **Visual Indicators:** Dates with scheduled events are highlighted with blue background
- **Drag & Drop Timeline:** Events can be dragged to different time slots with visual feedback
  - Drag handle appears on hover (Material icon: `drag_indicator`)
  - Drag preview shows event summary while dragging
  - Drop zones highlight when a draggable is active
  - Placeholder shows where event will land
- **Event Count:** Calendar sidebar shows count of events for selected date
- **Responsive Layout:** Two-column grid (320px calendar + flexible timeline)

**Demo Mode Support:**
When backend API returns empty response or is unavailable (common during HCM field visits), the `ScheduleEventService` automatically falls back to dummy data from `MockDataService`, ensuring seamless demonstration capabilities without backend dependency.
...
| 1.5 | Apr 2026 | Agent Narsingh | Enhanced Scheduling Module: Replaced static calendar with Angular Material `mat-calendar` + Angular CDK Drag & Drop; added visual date indicators for events; implemented drag-and-drop event rescheduling with visual feedback (drag handles, preview, placeholder); added demo mode support where `ScheduleEventService` falls back to `MockDataService` dummy data when API returns empty or fails (enables HCM demos without backend); updated `scheduling.component.ts`, `.html`, `.scss` with date selection, event filtering by selected date, and drag event handlers |
| 1.6 | Apr 2026 | Agent Narsingh | Enhanced Scheme List Module: Replaced PrimeNG components with Angular Material (`p-table` → `mat-table`, `p-select` → `mat-select`, `p-tag` → custom status badges, `p-dialog` → custom dialog with Material buttons); replaced PrimeNG icons with Material icons (`pi pi-plus` → `<mat-icon>add</mat-icon>`, `pi pi-list` → `<mat-icon>list</mat-icon>`, `pi pi-eye` → `<mat-icon>visibility</mat-icon>`); added demo mode support where `SchemeService` falls back to `MockDataService` dummy scheme applications when API returns empty or fails; implemented automatic scheme statistics calculation from loaded data (total, approved, pending, rejected, budget per scheme type); updated `scheme-list.component.ts`, `.html`, `.scss` with Material table, white background enforcement, and custom status badge styling |
| 1.7 | Apr 2026 | Agent Narsingh | Enhanced Dashboard Module: Fixed alignment issues in section headers by adding flexbox (`display:flex;align-items:center;gap:0.5rem`); replaced all PrimeNG icons with Material icons throughout dashboard (KPI cards: `pi-calendar-plus` → `event_available`, `pi-clock` → `schedule`, etc.; section headers: `pi-calendar` → `calendar_today`, `pi-history` → `history`, `pi-chart-pie` → `pie_chart`, `pi-chart-bar` → `bar_chart`, `pi-bolt` → `bolt`; recent activity icons: `pi-check-circle` → `check_circle`, `pi-arrow-right-arrow-left` → `swap_horiz`, `pi-user-plus` → `login`, `pi-bell` → `notifications`; schedule event location: `pi-map-marker` → `location_on`); replaced PrimeNG buttons with Material buttons in Quick Actions section (`p-button` → `mat-raised-button` with Material icons); added `matIcon` property to KPI objects and Quick Actions; updated `dashboard.component.ts`, `.html` with Material icon integration and proper alignment |
| 1.8 | Apr 2026 | Agent Narsingh | Enhanced Reports Module: Fixed chart sizing and alignment issues by optimizing chart dimensions to fit properly within containers (doughnut chart: `max-width:260px;max-height:260px`, bar charts: `max-height:240px`); replaced all PrimeNG icons with Material icons in section headers (`pi pi-chart-bar` → `<mat-icon>bar_chart</mat-icon>`, `pi pi-chart-pie` → `<mat-icon>pie_chart</mat-icon>`, `pi pi-map-marker` → `<mat-icon>location_on</mat-icon>`); replaced PrimeNG buttons with Material buttons in header action bar (`pButton` → `mat-stroked-button` with Material icons: `pi pi-map` → `map`, `pi pi-clock` → `schedule`, `pi pi-history` → `history`); added flexbox styling to all section headers for consistent alignment; added padding to chart containers to prevent edge clipping; updated `reports.component.ts`, `.html` with Material imports and icon integration |
| 1.9 | Apr 2026 | Agent Narsingh | Enhanced Visitor Dashboard with Demo Data: Added comprehensive dummy data directly in `visitor-dashboard.component.ts` `ngOnInit()` for immediate display without backend dependency; includes mock visitor profile (Rajesh Kumar Sharma with AADHAAR KYC, 94% confidence score, PHOTO_MATCHED status), 3 sample appointments (SCHEDULED, COMPLETED, CMO_REVIEW statuses), 3 scheme applications (APPROVED and UNDER_REVIEW with monetary amounts), 2 grievances (RESOLVED and UNDER_REVIEW), and 7 total visits count; KPI cards now dynamically display actual counts from dummy data arrays; enables full visitor dashboard demo functionality for presentations and testing without live API; fixed section header visibility by enforcing white text color on gradient background (`!important` rules); fixed analytics values visibility by ensuring dark text color (#1a237e) on white background |
| 2.0 | Apr 2026 | Agent Narsingh | Enhanced HCM Dashboard with AI-Powered Demo Data: Added comprehensive demo data in `dashboard.component.ts` via `initializeDummyData()` method with intelligent fallback logic (API data takes precedence, dummy data displays if API returns empty); includes 5 realistic schedule events with **AI-generated meeting summaries** (Cabinet briefing, file clearing, public durbar, individual appointments, state events) each with contextual descriptions, event types (A1-A4, B1-B2), locations, and time slots; added 5 recent activity log entries with action types (APPROVED, STATUS CHANGE, CREATED, LOGIN, REMINDER); populated KPI cards with realistic counts (12 appointments, 8 pending approvals, 34 active schemes, 5 follow-ups, 23 walk-ins, 6 CMO reviews); **enhanced schedule display** in `dashboard.component.html` to show AI summaries with purple sparkle icon (`auto_awesome`) in italic grey text below event titles; maintains API integration while providing rich demo experience for HCM presentations and testing |
| 2.1 | Apr 2026 | Agent Narsingh | Enhanced AI Insights Dashboard with Material UI and Expanded Demo Data: Replaced all PrimeNG icons with Material icons (`pi-sparkles` → `auto_awesome`, `pi-briefcase` → `business_center`, `pi-map` → `location_city`, `pi-tag` → `label`, `pi-info-circle` → `info`, `pi-spin pi-spinner` → `progress_activity`); added `MatIconModule` import; significantly expanded `getMockInsights()` demo data: increased total applications from 247 to 662 matching heatmap totals, expanded district coverage from 6 to all 11 Meghalaya districts (East Khasi Hills 142, West Garo Hills 118, East Garo Hills 87, West Khasi Hills 64, Ri Bhoi 56, South Garo Hills 48, West Jaintia Hills 39, East Jaintia Hills 35, North Garo Hills 31, South West Khasi Hills 24, Eastern West Khasi Hills 18), enhanced scheme names with full descriptions (CMSDF Chief Minister Special Development Fund, CM Care Medical Assistance, CMSG Rural Infrastructure Grant, CM Elevate Youth Entrepreneurship, CM Connect Digital Infrastructure), expanded categories from 5 to 8 (Road & Connectivity 158, School/Education 124, Medical 112, Community Hall 89, Water Supply 67, Electricity 52, Agricultural 38, Youth Entrepreneurship 22), and enriched AI note with detailed analysis including 23% surge, district leader identification, predictive modeling ("180+ new applications expected next week"), trend insights, and actionable recommendations; ensures Material UI consistency across all dashboard components |
| 2.2 | Apr 2026 | Agent Narsingh | Integrated reference data API for scheme types in scheme-form component, replacing hardcoded values with dynamic CM_SCHEME data from ReferenceDataController |
| 2.3 | Apr 2026 | Agent Narsingh | Refactored scheme types loading to use SchemeService instead of direct HttpClient calls in scheme-form component for better separation of concerns |
| 2.4 | Apr 2026 | Agent Narsingh | Fixed environment imports across all services and components to use './environments/environment' for proper build-time environment switching instead of hardcoded development imports |
| 2.5 | Apr 2026 | Agent Narsingh | Added Home navigation and explicit Previous/Next step controls to visitor registration KYC flow in `visitor-register.component.html` |
| 2.6 | Apr 2026 | Agent Narsingh | Completed comprehensive Person to Visitor entity refactoring: renamed Person.java to Visitor.java with table name "visitors", updated all entity relationships (Appointment.applicant, AssociateMapping.person, SchemeApplication.applicant), renamed PersonRepository to VisitorRepository with updated queries, renamed PersonController to VisitorController with /api/v1/visitors endpoints, renamed PersonDto to VisitorDto, created V13 database migration to rename persons table to visitors, moved registration logic from VisitorAuthController to VisitorService.registerVisitor() method with comprehensive validation (phone format, EPIC/Aadhaar patterns, duplicate checking), updated VisitorAppointmentController and AppointmentService to use VisitorRepository, resolved all compilation errors and ensured successful Maven build |
| 2.7 | Apr 2026 | Agent Narsingh | Converted User Management Admin Component from PrimeNG to Angular Material: replaced p-table with mat-table (displayedColumns array for dynamic columns), p-select with mat-select (mat-option for role dropdown), p-password with mat-input (toggle password visibility with mat-icon button), p-button with mat-raised-button and mat-icon-button, p-dialog with mat-dialog-container (mat-dialog-content and mat-dialog-actions), p-tag with mat-style badges (ngStyle for dynamic role colors: red for HCM, amber for ADMIN, blue for APPROVER roles, grey for DEO), replaced all PrimeNG icons (pi pi-*) with Material icons (group, add, list, edit, delete, error, check_circle, visibility, visibility_off), updated component imports to use Material modules, added pageSizeOptions and displayColumns arrays for table configuration, updated roleBadge() method to return style object for ngStyle binding, ensured white background on all form inputs (mat-form-field appearance="outline"), successful frontend build with all Material components compiled |
| 2.8 | Apr 2026 | Agent Narsingh | Implemented Dynamic Document Management System for Appointment Form: Added DocumentUpload interface to track document type, label, visibility, upload status, and content; created documents array with 9 document types (EPIC_SCAN, APPLICATION_LETTER, PLANS_ESTIMATES, BANK_DETAILS, MLA_APPROVAL_LETTER, ORG_REGISTRATION_CERTIFICATE, CM_CARE_ELIGIBILITY, CM_CARE_HOSPITAL, CM_CARE_SUPPORTING); implemented `updateDocumentVisibility()` method to conditionally display documents based on form state (EPIC scan visible if KYC pending, Plans visible for schemes, MLA approval visible if selected, Organisation cert visible if org selected, CM Care docs visible if CM Care scheme selected); added `onDocumentChange()` method to handle file uploads with filename tracking and visual confirmation; replaced static document upload section with dynamic `*ngFor` loop generating upload UI for each visible document; added comprehensive Document Requirements Summary table displaying document type, requirement status (required/optional/not applicable), upload status with visual indicators (✓ Uploaded, Pending, Optional, Not Required); added document validation in submit() method checking all visible required documents are uploaded before submission, with user-friendly error messages listing missing documents; updated submit() to use FormData for multipart file uploads with proper document field naming; added (change) handlers to agendaType, schemeType, mlaMdcApproved, and isOrganisation form fields to trigger visibility updates in real-time; updated `ngOnInit()` to call `updateDocumentVisibility()` for initial document state; ensures white backgrounds on all inputs and Material button styling for upload buttons per UI/UX mandates; enables flexible, schema-driven document requirements that adapt to application type and user profile |
| 2.9 | Apr 2026 | Agent Narsingh | Fixed AppointmentController to Correctly Save Appointments with Document Uploads: Created DocumentUpload entity with appointment/visitor/scheme relationships and document metadata fields (documentType, originalFilename, filePath, fileSizeBytes, mimeType, uploadedBy, timestamps); created DocumentUploadRepository with query methods for finding documents by appointment/visitor and document type; completely refactored VisitorAppointmentController from expecting JSON (@RequestBody) to properly handling multipart/form-data with @RequestParam parameters for form fields and HttpServletRequest for accessing file parts; updated submitAppointment() method to extract multipart form data using request.getParts(), properly parse file uploads with dynamic parameter names (documents_*), convert servlet Part objects to MultipartFile using new convertPartToMultipartFile() helper method, call FileStorageService.storeFile() to persist files, and create DocumentUpload records in database linking files to appointments; added proper error handling for document uploads (logs errors but continues processing), JSON parsing for associates array, and multipart request retrieval; enables complete end-to-end document upload workflow critical for appointment scheme applications |
| 3.0 | Apr 2026 | Agent Narsingh | Enhanced Scheme Form with Missing Appointment Scheme Fields: Added schemeHistoryList array to form object to track schemes applied in last 2 years; added schemeHistoryOptions array with full list of CM schemes (CMSDF, CMSG, CM Care, CM Connect, CM Elevate, Focus+); implemented `isSchemeInHistory(scheme)` and `toggleSchemeHistory(scheme)` methods for dynamic checkbox selection management; added "Schemes taken in last 2 years" section to Step 2 (Financial Details) with multi-select checkboxes styled identically to appointment form (with colors changing on selection); added MatCheckboxModule import to component; ensures scheme form captures all required data points consistent with appointment workflow: Community Contribution field (already present), Estimated Cost via itemized financial breakdown, and scheme history tracking; enables proper standalone scheme application submissions with complete information parity to appointment-based scheme requests |
| 3.1 | Apr 2026 | Agent Narsingh | Fixed Required Documents Section Label Visibility Issue: Added explicit `color: #1f2937;` CSS property to `.doc-label` class in scheme-form.component.scss; document labels in Step 3 (Required Documents) were not visible due to missing text color specification; now displays document labels (Plans & Estimates, Bank Account Details, MLA/MDC Letter, Hospital Docs, Organisation Certificate) in dark grey ensuring proper contrast and readability; verified build success with no compilation errors |
| 3.2 | Apr 2026 | Agent Narsingh | **Implemented Admin Scheme Management Module (ADMIN-ONLY ACCESS)**: Created SchemeDocument JPA entity and repository to store configurable supporting document requirements per scheme (references reference_data.code for scheme_code instead of redundant schemes table - reusing existing CM_SCHEME reference data); created SchemeDto and SchemeDocumentDto DTOs for API communication; implemented SchemeController (/api/v1/admin/schemes - ADMIN-ONLY role-based authorization) with endpoints: GET all schemes, GET by code, POST create new scheme, PUT update scheme active/inactive status, PUT configure documents for scheme; added ReferenceDataRepository query methods (findByTypeOrderByDisplayOrder, findByTypeAndCode, findMaxDisplayOrder) to support scheme queries from reference_data table; created V16 database migration adding scheme_required_documents table with unique constraint on (scheme_code, document_type), indexes on scheme_code and display_order; seeded sample document requirements for CMSDF, CMSG, and CM_CARE schemes with file formats; created SchemeManagementComponent (standalone Angular 19 + Material) with features: list all schemes with active/inactive toggle (mat-slide-toggle), create new scheme via form, configure required documents per scheme with CRUD operations (add/remove document rows), document type/label/required/format/order management in table layout, dynamic document type dropdown, file format options, Material table for UI consistency with white backgrounds and proper styling; all admin operations log username via JwtUtils for audit trail; endpoints properly enforce ADMIN role check returning 403 if non-admin attempts access; enables government admins to manage scheme catalog and define document submission requirements without code changes or database direct access |
| 3.3 | Apr 2026 | Agent Narsingh | **Implemented Comprehensive Appointment Type Configuration System**: Expanded AppointmentTypeConfig entity from 15 to 70+ fields organized in 7 functional categories: (1) Type Definition (requiresTravel, travelTimeBefore/After, blockTimeIncludes, hasAppointmentLimit, maxAppointmentLimit, limitIsSacrosanct, generateAlerts, noTravelTime), (2) Calendar Configuration (calendarColor, blockCalendarSlot, availabilityWindows, blackoutDates as JSON), (3) Direct Scheduling Rules (allowDirectScheduling, directSchedulingRoles, bypassApprovalProcess, approverBypassRoles), (4) Conflict Handling (detectConflicts, allowConflictOverride, conflictOverrideRoles, notifyOnConflict, conflictNotificationTemplate), (5) Batch Scheduling Type B specifics (isBatchType, maxParticipants, requiresPreApproval, allowWalkIn, maxWalkInCount, autoAssignSeat), (6) Waiting List Management (enableWaitingList, maxWaitingListSize, autoAddFromWaitingList, waitingListNotificationDays), (7) OSD Special Role (osdCanOverride, osdCanBypassLimits, osdCanDirectSchedule), (8) Drag-and-Drop Scheduling (allowDragDropRescheduling, dragDropAllowedRoles, validateConflictsOnDragDrop), plus Status fields (isActive, displayOrder, createdBy, updatedBy); created comprehensive AppointmentTypeDto matching all 70+ properties; created AppointmentTypeService with 8 methods providing CRUD operations, role-based access control, category filtering, and DTO conversion; created AppointmentTypeController (/api/v1/admin/appointment-types - ADMIN-ONLY) with 6 REST endpoints (GET all types, GET by code, GET by category, POST create, PUT update, PATCH toggle status); created AppointmentTypeConfigRepository with 6 custom query methods (findAllActive, findByTypeCategory, findMaxDisplayOrder, findTravelRequiredTypes, findTypesWithLimits, findDirectSchedulingTypes); created V17 database migration creating appointment_type_configs table with 70+ columns including seed data for 6 appointment types (A1, A2, A3, A4, B1, B2) with default configurations matching SRS specifications; created AppointmentTypeManagementComponent (standalone Angular 19 + Material) enabling admins to view all appointment types with expandable Material panels, edit type configurations (travel time, limits, calendar colors, scheduling rules, batch settings, waiting list options, OSD powers), toggle active/inactive status, Material table display with category badges and color-coding; updated AppointmentTypeDto to full 40+ properties; fixed AppointmentTypeController ResponseEntity type inference errors by replacing map/orElse Optional handling with explicit if-else logic; verified backend Java compilation successful with `mvn clean compile`; verified frontend Angular 19 build successful; added route /admin/appointment-types with ADMIN role guard; added "Appointment Types" menu item to shell component sidebar; enables complete admin interface for configuring all appointment type characteristics (travel requirements, limits, calendar colors, approval rules, batch scheduling parameters, waiting list behavior, OSD overrides) critical for scheduling engine adaptation across the 6 appointment type categories |
