...
| REG-APPT-UI-001 | Angular Appointments | View History icon rendering | Open All Appointments, Walk-in Appointments, and Closed Appointments at desktop and narrow widths; inspect and click the History action. | The complete history icon is visible, is not clipped, has an accessible label/tooltip, and opens visitor history. | High | UI QA | Yes |
| REG-APPT-UI-002 | Angular Appointments | Appointment Source mapping | Load records where `appointmentType=APPOINTMENT`, `appointmentSource=CITIZEN` and `appointmentType=B2 Walk-in`, `appointmentSource=WALKIN`; inspect the Appointment Source column. | The column displays `CITIZEN` and `WALKIN` respectively from `appointmentSource`; it never displays `APPOINTMENT` or `B2 Walk-in`. | High | UI/API QA | Yes |
| REG-APPT-UI-003 | Angular Appointments | Appointment Source sorting | Sort the Appointment Source column ascending and descending. | Sorting uses `appointmentSource` and matches the values displayed in the column. | Medium | UI QA | Yes |
| REG-APPT-MOB-001 | Mobile Appointments | History action and source mapping | Open an appointment card and detail view; inspect History and Appointment Source. | History icon/button is fully visible and opens the history sheet; source is read from `appointmentSource`, independently of `appointmentType`. | High | Mobile QA | Yes |
