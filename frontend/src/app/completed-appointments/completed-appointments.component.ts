...
export class CompletedAppointmentsComponent implements OnInit,OnDestroy {
  ...
  addFollowUp(){const d=this.detail;if(!d)return;const remarks=window.prompt('Follow-up remarks');if(!remarks?.trim())return;this.appointments.addRemark(d.appointment.id,{hcmRemarks:remarks.trim()}).subscribe({next:()=>{this.toast.success('Follow-up remarks saved successfully.');this.view(d.appointment.id);},error:()=>this.toast.error('Unable to save follow-up remarks.')});}
  closeAppointment(){const d=this.detail;if(!d||!window.confirm('Close this appointment? This makes the record final.'))return;const remarks=window.prompt('Final closing remarks (optional)')||'';this.appointments.closeAppointment(d.appointment.id,remarks).subscribe({next:()=>{this.toast.success('Appointment closed successfully.');this.closeDetail();this.load();},error:()=>this.toast.error('Unable to close appointment.')});}
